...
class DiffusionOPT():
    ...
    def select_action(self, state, i_episode):
        state = torch.from_numpy(state).float().unsqueeze(0).to(self._device)  # ✅ Move to same device
        logits = self._actor(state).detach().cpu().numpy()  # ✅ Optional: move back to CPU if needed
        logits = np.clip(logits, -1, 1)  # Simpler form of your clipping logic
        return logits[0]
